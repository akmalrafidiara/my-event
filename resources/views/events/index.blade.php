                ...
                <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 14px;">
                    <thead>
                        <tr style="background-color: #ea580c; color: white;">
                            ...
                            <th style="padding: 16px;">Start At</th>
                            ...
                            <tr style="border-bottom: 1px solid #e5e7eb; transition: background-color 0.3s;" onmouseover="this.style.backgroundColor='#f3f4f6';" onmouseout="this.style.backgroundColor='white';">
                                ...
                                <td style="padding: 16px;">
                                    <img src="{{ Storage::url($event->featured_image) }}"
                                         alt="{{ $event->title }}"
                                         style="height: 80px; width: 80px; object-fit: cover; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                                </td>
                                ...
                                <td style="padding: 16px;">{{ $event->start_event_at }}</td>
                                <td style="padding: 16px; text-align: center;">{{ $event->registrant_count }}</td>
                                <td style="padding: 16px; text-align: center;">
                                    <div style="display: flex; justify-content: center; gap: 10px;">
                                        <a href="{{ route('events.show', $event->id) }}"
                                           style="background-color: #2563eb; color: white; padding: 8px 16px; border-radius: 8px; text-decoration: none; font-weight: 600; transition: background-color 0.3s;"
                                           onmouseover="this.style.backgroundColor='#1d4ed8';" onmouseout="this.style.backgroundColor='#2563eb';">
                                            View
                                        </a>
                                        @if(Auth::user()->role == 'admin')
                                        <a href="{{ route('events.edit', $event->id) }}"
                                           style="background-color: #22c55e; color: white; padding: 8px 16px; border-radius: 8px; text-decoration: none; font-weight: 600; transition: background-color 0.3s;"
                                        ...
                                        <form action="{{ route('events.destroy', $event->id) }}" method="POST" style="margin: 0; display: inline;">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                    style="background-color: #dc2626; color: white; padding: 8px 16px; border-radius: 8px; font-weight: 600; border: none; cursor: pointer; transition: background-color 0.3s;"
